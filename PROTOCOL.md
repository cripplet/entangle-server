...
`DENIED` error is returned if the request `AUTH` token does not match the token kept on the server.
...
`UNSYNC` error occurs later with `AUX` set to the saved `MSG_ID`, the client, upon receiving a response, does **not** need to re-sync the `AUX` field of response to 
