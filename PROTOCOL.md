...
| ----- | ----- |
| `CMD` | mirrored |
| `AUX` | `F:C:D`, where `F` is the file offset of the beginning of the buffer, `C` is the current client position, and `D` is the data |
...
it has synced with the server with last `SYNC` of `MSG_ID` -- if an `UNSYNC` error occurs later with `AUX` set to the saved `MSG_ID`, the client, upon receiving 
a response-1, does **not** need to re-sync the `AUX` field of response-1 to itself, but **does** need to update the `MSG_ID` for the id of the current `SYNC` taking 
place.
