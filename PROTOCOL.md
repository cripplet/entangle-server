...
* `MSG_ID` -- Unique *numeric* message id for each packet; the client and server have **separate** message ids (cf. TCP protocol) which will **both** be incremented 
	seperately. Upon receiving a packet, the server or client (the recipient) will **drop** packet if the packet `MSG_ID` does not match the previous recieved packet
	`MSG_ID` incremented by one.
...
be fulfilled by the server due to various reasons. A list of possible error codes to be generated by the relevant response is included in this section. In addition, a 
`DENIED` error is returned if the request `AUTH` token does not match the token kept on the server, `UNIMPL` returned if the command is not recognized, or `INVALID` on 
invalid format.
...
| Field | Value |
| ----- | ----- |
| `MSG_ID` | initial client-side message id |
| `CLIENT_ID` | blank |
| `AUTH` | blank or alphanumeric token |
| `CMD` | `CONN` |
| `AUX` | `H:P:T`, where `H` is the hostname, `P` is the client port, and `T` an optional alphanumeric server token |

### Response

| Field | Value |
| ----- | ----- |
| `MSG_ID` | initial server-side message id |
...
Server will **ignore** raising `UNEXPECTED` error here -- a client can always disconnect from the server.
...
A response may be generated by the server spontaneously WITHOUT prompting by the client.
...
The client, upon receiving the server response, MUST generate an `ACK` packet. The server, upon receiving response-ack, will update its internal client data buffer to 
match that of the client. The client, upon sending response-ack will update itself that it has synced with the server with last `SYNC` of server-side `MSG_ID` -- if an 
`DESYNC` error occurs later with `AUX` set to the saved `MSG_ID`, the client, upon receiving a response, does **not** need to re-sync the `AUX` field of response to 
itself, but **does** need to update the `MSG_ID` for the id of the current `SYNC` taking place.
...
Server-generated packets -- the client MUST acknowledge a packet if the given client id in the `AUX` field matches its own -- otherwise, an `DESYNC` error will be 
...
* `DESYNC` -- the client has not acknowledged a server `SYNC` response packet; a `SYNC` packet will be sent immediately after the error (to which the client must 
	respond)
...
| `CMD` | mirrored |
| `ERR` | error code |
| `AUX` | `SYNC_MSG_ID:SYNCPOS_MSG_ID` of last successful `SYNC` and `SYNCPOS` if error code is `DESYNC`, otherwise blank |
...
| ---- | ----------- | ----------- |
| 0 | `NO_ERR` | no error |
| 400 | `UNEXPECTED` | missing packet |
| 401 | `DENIED` | access denied |
| 404 | `UNIMPL` | command not recognized |
| 406 | `INVALID` | invalid arguments |
| 409 | `DESYNC` | client is out of sync with server -- server will follow up with a `SYNC` and `SYNCPOS` packet |
