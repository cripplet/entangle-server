...
| Field | Value |
| ----- | ----- |
| `MSG_ID` | **non-zero** initial message id |
| `CLIENT_ID` | blank |
...
A response can be generated by the server spontaneously WITHOUT prompting by the client. In this case, `MSG_ID` is set to a unique server-side value.

| Field | Value |
| ----- | ----- |
| `CMD` | mirrored |
| `AUX` | `F:D`, where `F` is the file offset of the beginning of the buffer and `D` is the data |

### Response-2

The client, upon receiving the server response, MUST generate an `ACK` packet, with `MESSAGE_ID` set to the same id as the original request or spontaneous response. The 
server, upon receiving response-2, will update its internal client data buffer to match that of the client. The client, upon sending response-2 will update itself that 
it has synced with the server with last `SYNC` of `MESSAGE_ID` -- if an `UNSYNC` error occurs later with `AUX` set to the saved `MESSAGE_ID`, the client, upon receiving 
a response-1, does **not** need to re-sync the `AUX` field of response-1 to itself, but **does** need to update the `MESSAGE_ID` for the id of the current `SYNC` taking 
place.
...
| `CMD` | mirrored |
| `ERR` | error code |
| `AUX` | `MESSAGE_ID` of last successful `SYNC` if error code is `UNSYNC`, otherwise blank |
